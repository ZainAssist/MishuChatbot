...
import psutil
import config
from MishuChatbot import _boot_
from MishuChatbot import get_readable_time
from MishuChatbot import MishuChatbot, mongo
...
from MishuChatbot.database.chats import get_served_chats, add_served_chat
from MishuChatbot.database.users import get_served_users, add_served_user
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery
from MishuChatbot.modules.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
STICKER = [
    "CAACAgUAAxkBAAEO02RoY8Fn_K872N44Y9CfsfWaESGenQACJRIAAg0kyFdEoqS1HhU3xjYE",
    "CAACAgUAAxkBAAEO02ZoY8Fy9rs2mqeP3Wf-MZQYp-KL9AAC9BQAApvY6FSRa2PH3VpVUDYE",
    "CAACAgUAAxkBAAEO02hoY8F-JPvVsVprRkfOsy-3xubUZQACbBcAAqPnyFeybHQPolxy5TYE",
]
...
IMG = [
    "https://graph.org/file/d97ee31fb34d0a9b3a958-9025a8dab9a68bb3ce.jpg",
    "https://graph.org/file/60c6c87be1f1fb3cf4b1b-2c65287a7744316907.jpg",
    "https://graph.org/file/272610985cc3a83a93c12-f258d6104db16b53e3.jpg",
    "https://graph.org/file/70e1254c01dc98c445d0b-3b28290861b123720b.jpg",
    "https://graph.org/file/25b4f68b6dd4f25ccb45a-393329a3d329d7481c.jpg",
    "https://graph.org/file/cd28d1e128e423113b2c7-f91c59664ffa75aad5.jpg",
    "https://graph.org/file/92a5a22c6116251ba633a-f751cd2fa071a8371d.jpg",
    "https://graph.org/file/a422c6e6195ca529b6f89-f475e6faee90bc333d.jpg",
    "https://graph.org/file/5c679cad86852782955e9-517e9412383c7544b1.jpg",
    "https://graph.org/file/4366618b13e2fc50b8681-db7197b8a61a2af7a9.jpg",
    "https://graph.org/file/a4adb7499769f3d87216f-973db11e6e96a15951.jpg",
    "https://graph.org/file/1aee3184ce9b54cbef117-0d5ebc0c3c3b363c3b.jpg",
    "https://graph.org/file/de9925cf068813e9af50a-6aa1cba6917c42be77.jpg",
    "https://graph.org/file/7acde6abf352a9e3e4101-e1be0d9d281a512a39.jpg",
]



from MishuChatbot import db
...
@MishuChatbot.on_message(filters.new_chat_members)
        ...
        for member in message.new_chat_members:
            ...
            if member.id == MishuChatbot.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await MishuChatbot.export_chat_invite_link(message.chat.id)
                ...
                count = await MishuChatbot.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
                username = chat.username if chat.username else "𝐏ʀɪᴠᴀᴛᴇ 𝐆ʀᴏᴜᴘ"
                msg = (
                    f"**📝𝐂ʜᴀᴛ 𝐁ᴏᴛ 𝐀ᴅᴅᴇᴅ 𝐈ɴ 𝐀 #𝐍ᴇᴡ_𝐆ʀᴏᴜᴘ**\n\n"
...
@MishuChatbot.on_cmd(["start", "aistart"])
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await MishuChatbot.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(MishuChatbot.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await MishuChatbot.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@MishuChatbot.on_cmd("help")
async def help(client: MishuChatbot, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{MishuChatbot.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve  αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||** ⋆ʟᴏᴠᴇ ᴡɪᴛʜ⋆ [ZAIN𓆪](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@MishuChatbot.on_message(filters.command("stats"))
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await MishuChatbot.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await MishuChatbot.send_message(
